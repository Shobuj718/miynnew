...
<html xmlns="http://www.w3.org/1999/xhtml">
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0" style="mso-table-lspace:0pt;mso-table-rspace:0pt;">
									<tbody>
										<tr>
											...
											<div style="width: 70px;height: 70px;font-size: 20px;color: #ffffff;line-height: 70px;margin:0 auto;border-radius: 50px;background-color: #4675d9;">{{ substr($booking->staff_name, 0, 1) }}</div>        
											<!-- } -->
											</td>
										</tr>
									</tbody>
	...
	<div style="line-height: 2;">Dear {{ strtoupper($booking->staff_name) }}, </div>
	<div style="line-height: 2;">{{ ucfirst($booking->first_name)." ".ucfirst($booking->last_name) }} has requested an appointment with you. Please <a style="color: blue;" href="{{ env('APP_URL') }}/login">Login Here</a> to Accept or Deny this request.</div>
											...
											<div style="text-align: center;">
												...
												@foreach ($booking2 as $key => $value)
													...
													?>
												        <span style="font-size: 20px">{{ $key }} at</span>
											...
											<br>
											<div style="text-align: center;">
											  <a style="color:#fff;" href="{{ env('APP_URL') }}/login"><h3 style="width: 270px;height: 30px;background-color: #26db28;padding-top: 5px;border-radius: 4px;margin-left:140px;">
											  	Accept
											  </h3></a>
											    <a href="{{ env('APP_URL') }}/login">Decline</a>
											    <br>
											    <a href="{{ env('APP_URL') }}/login">Reply / View Online</a>
											</div>
											<br>
											<div>
											    Full details on <a href="{{ env('APP_URL') }}/login">{{ $booking->first_name." ".$booking->last_name }}</a>
											</div>
											<div>Email: {{$booking->email}}</div>
											<div>Email: {{$booking->phone}}</div>
