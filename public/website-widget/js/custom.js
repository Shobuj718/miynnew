...
var today = new Date(newDate.getFullYear(), newDate.getMonth(), newDate.getDate(), 0, 0, 0, 0);
...
(function ($) {
	...
	getCalendar = function (target_div, year, month) {
		...
		$.ajax({
			type: 'get',
			url: 'http://127.0.0.1:8000/api/get-calendar?key=' + key,
...
// 		console.log("Screen Date: ", screenDate);
	...
	updateCalendarSettings = function (slug) {
		$.ajax({
			type: 'get',
			url: 'http://127.0.0.1:8000/api/get-calendar-settings?slug=' + slug
		}).done(function (data) {
	...
	$(document).ready(function ($) {
		...
		$(document).on('click', '#miyn-client-widget a', function (e) {
			...
			if (aLink.startsWith('#')) {
				...
				if (aLink === "#appointment-box") {
					...
					$.ajax({
						url: 'http://127.0.0.1:8000/api/categories?key=' + key,
						method: "get"
					}).done(function (data) {
						console.log(data);
						var servicesHtml = '';
				...
				if (aLink === "#staff-box") {
					$.ajax({
						url: 'http://127.0.0.1:8000/api/staffs?key=' + key,
						method: "get"
					}).done(function (data) {
						console.log(data);
						var staffListHtml = '';
		...
		$(document).on('click', "#book-service", function() {
		    ...
		    infoCenter['email'] = $('#email').val();
		    infoCenter['country_code'] = $('#country_code').val();
		    ...
		    var country_code = infoCenter['country_code'];
		    ...
		    $.ajax({
				...
				servicesHtml += '<a style="margin-left:5px;color:#fff;" class="btn btn-info" target="_blank" onclick="window.location.href = \'http://127.0.0.1:8000/login\';">Manage Booking</a>';
				servicesHtml += '<p style="text-align:center;margin:60px 0px 20px 0px;">Online Scheduling by <a style="cursor:pointor;color:blue;" onclick="window.location.href = \'http://127.0.0.1:8000/\';">Miyn</a></p>';
				servicesHtml += '</div>';
				servicesHtml += '</div>';
